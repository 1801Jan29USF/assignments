...
public class AtmScreen implements Screen {
	...
	public Screen prompt() {
			...
			try {
				...
				while (deposit <= 0) {
					...
					try {
						...
						System.out.println(
								"That kind of moeny is not allowed here");
						System.out.println("Try again!");
						//timeOut();
						scan.nextLine();
						return this.prompt();
		...
		case "2":
			...
			try {
				...
				while (withdraw <= 0) {
					System.out
							.println("SlickRick's Financial Insitution does not allow withdraws equal or less than 0 .");
					System.out.println("Please enter a valid amount:");
					try {
						withdraw = scan.nextDouble();
						scan.nextLine();
	...
	public Screen otherTransaction() {
		...
		System.out.println("Please go back to the GUI");
		return null;
	}
